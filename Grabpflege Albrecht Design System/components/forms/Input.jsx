...
export function Input({ label, hint, error, required, type = "text", icon, style, wrapperStyle, ...rest }) {
  const [focus, setFocus] = React.useState(false);
  return (
    <Wrap label={label} hint={hint} error={error} required={required} style={wrapperStyle}>
      <span style={{ position: "relative", display: "block" }}>
        {icon ? <span style={{ position: "absolute", left: 12, top: "50%", transform: "translateY(-50%)", color: "var(--text-subtle)", display: "flex" }}>{icon}</span> : null}
        <input type={type} onFocus={() => setFocus(true)} onBlur={() => setFocus(false)} {...rest}
          style={{ ...control(!!error, focus), paddingLeft: icon ? 38 : undefined, ...style }} />
      </span>
    </Wrap>
  );
}
